...
#' Nothing is ever inverted, and the Cholesky factor keeps the sparsity of Q,
#' so the cost is that of the factorisation rather than of a dense inverse and
#' a dense n-by-n Cholesky of it. That matters for a model with a Gaussian
#' field, where Q is the joint precision over thousands of latent variables: on
#' a 4800-node mesh, inverting and drawing 1000 samples takes about 20 seconds
#' and 180 MB, against 0.2 seconds through the factorisation. A dense precision
#' is simply a sparse matrix with no zeros, and costs no more this way than the
#' old route did.
#' 
