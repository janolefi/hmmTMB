...
MarkovChain <- R6Class(
  ...
  public = list(
    ...
    initialize = function(data = NULL,
      ...
      # Formulas are rebuilt from strings below, which loses the environment
      # they were written in. Objects a smooth refers to -- the mesh of an SPDE
      # field, say -- have to stay findable, so keep the environment of the
      # formula, or the frame that called MarkovChain$new() when the formulas
      # were given as a character matrix (R6 puts one frame in between).
      form_env <- if(inherits(formula, "formula")) environment(formula) else parent.frame(2)
      ...
      ls_form <- lapply(ls_form_char, function(form_char) {
        as.formula(ifelse(form_char == ".", "~1", form_char), env = form_env)
      })
